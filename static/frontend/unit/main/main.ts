...
import {
  SelectNavController,
  makeSelectNav,
} from 'static/shared/outline/select';
...
const directories = document.querySelector<HTMLTableElement>(
  '.js-expandableTable'
);
if (directories) {
  ...
  const internalToggle = document.querySelector<HTMLButtonElement>(
    '.js-showInternalDirectories'
  );
  if (internalToggle) {
    if (document.querySelector('.UnitDirectories-internal')) {
      ...
      internalToggle.setAttribute(
        'aria-describedby',
        'showInternal-description'
      );
    }
...
const mobileNavSelect = document.querySelector<HTMLSelectElement>(
  '.DocNavMobile-select'
);
if (
  readme &&
