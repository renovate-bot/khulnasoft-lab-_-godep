...
export class CarouselController {
  ...
  private initDots() {
    ...
    for (let i = 0; i < this.slides.length; i++) {
      ...
      button.innerHTML = `<span class="go-Carousel-obscured">Slide ${
        i + 1
      }</span>`;
  ...
  private initLiveRegion() {
    ...
    this.liveRegion.textContent = `Slide ${this.activeIndex + 1} of ${
      this.slides.length
    }`;
  ...
  private setActive = (index: number) => {
    ...
    this.liveRegion.textContent =
      'Slide ' + (this.activeIndex + 1) + ' of ' + this.slides.length;
  };
}
