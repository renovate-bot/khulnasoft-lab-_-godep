...
export function initModals(): void {
  ...
  const jumpBody = jumpDialog?.querySelector<HTMLDivElement>(
    '.JumpDialog-body'
  );
  const jumpList = jumpDialog?.querySelector<HTMLDivElement>(
    '.JumpDialog-list'
  );
  const jumpFilter = jumpDialog?.querySelector<HTMLInputElement>(
    '.JumpDialog-input'
  );
    ...
    if (filter) {
      ...
      for (const item of jumpListItems ?? []) {
        ...
        if (nameLowerCase === filterLowerCase) {
          ...
          if (index > -1) {
            item.link.innerHTML = makeLinkHtml(
              item,
              index,
              index + filter.length
            );
  ...
  const shortcutsDialog = document.querySelector<HTMLDialogElement>(
    '.ShortcutsDialog'
  );
