...
export function registerHeaderListeners(): void {
  ...
  menuItemHovers.forEach((menuItemHover) => {
    ...
    const closeSubmenuOnEsc = (e: Event) => {
      ...
      const target = e.target as HTMLElement;
      if (event.key === 'Escape') {
        const forcedOpenItem = document.querySelector(
          '.forced-open'
        ) as HTMLElement;
        if (forcedOpenItem) {
  ...
  scrim?.addEventListener('click', (e) => {
    ...
    const activeSubnavs = document.querySelectorAll(
      '.go-NavigationDrawer-submenuItem.is-active'
    );
    activeSubnavs.forEach((subnav) =>
      handleNavigationDrawerInactive(subnav as HTMLElement)
    );
  ...
  const getNavigationDrawerMenuItems = (
    ...
    const anchorEl = navigationDrawer.querySelector(
      '.go-NavigationDrawer-header > a'
    );
    if (anchorEl) {
  ...
  const getNavigationDrawerIsSubnav = (navigationDrawer: HTMLElement) => {
    ...
    return navigationDrawer.classList.contains(
  ...
  const handleNavigationDrawerInactive = (navigationDrawer: HTMLElement) => {
    ...
    if (menuItems && menuItems[0]) {
      menuItems[0].removeEventListener(
        'keydown',
        handleMenuItemTabLeftFactory(navigationDrawer)
      );
