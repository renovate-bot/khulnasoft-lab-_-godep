...
describe('ExpandableRowsTableController', () => {
  ...
  it('sets data-aria-* and data-id attributes to regular html attributes', () => {
    ...
    expect(
      document.querySelector(
        '[aria-controls="hidden-row-id-1 hidden-row-id-2"]'
      )
    ).toBeTruthy();
    expect(
      document.querySelector('[aria-labelledby="toggle-id-1 label-id-2"]')
    ).toBeTruthy();
  ...
  it('hides rows with unexpanded toggles', () => {
    expect(document.querySelector('#hidden-row-id-1').classList).toContain(
      'hidden'
    );
    expect(document.querySelector('#hidden-row-id-2').classList).toContain(
      'hidden'
    );
  });
