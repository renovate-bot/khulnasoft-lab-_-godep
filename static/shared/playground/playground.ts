...
export class PlaygroundExampleController {
  ...
  constructor(private readonly exampleEl: HTMLDetailsElement) {
    ...
    this.playButtonEl = exampleEl.querySelector(
      PlayExampleClassName.PLAY_BUTTON
    );
    this.shareButtonEl = exampleEl.querySelector(
      PlayExampleClassName.SHARE_BUTTON
    );
    this.formatButtonEl = exampleEl.querySelector(
      PlayExampleClassName.FORMAT_BUTTON
    );
    this.runButtonEl = exampleEl.querySelector(PlayExampleClassName.RUN_BUTTON);
    this.inputEl = this.makeTextArea(
      exampleEl.querySelector(PlayExampleClassName.EXAMPLE_INPUT)
    );
    this.outputEl = exampleEl.querySelector(
      PlayExampleClassName.EXAMPLE_OUTPUT
    );
  ...
  private getCodeWithModFile(): string {
    ...
    const moduleVars =
      document.querySelector<HTMLDivElement>('.js-playgroundVars')?.dataset ??
      {};
      ...
      .then(async ({ Events, Errors }) => {
        ...
        for (const e of Events || []) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, e.Delay / 1000000)
          );
        }
      })
      .catch((err) => {
...
export function initPlaygrounds(): void {
  ...
  if (exampleHashRegex) {
    const exampleHashEl = document.getElementById(
      exampleHashRegex[1]
  ...
  const exampleHrefs = [
    ...document.querySelectorAll<HTMLAnchorElement>(
      PlayExampleClassName.PLAY_HREF
    ),
  ];
  ...
  for (const el of document.querySelectorAll(
    PlayExampleClassName.PLAY_CONTAINER
  )) {
    // There should be the same amount of hrefs referencing examples as example containers.
    const playContainer = new PlaygroundExampleController(
