...
`);
  ...
  it('opens playground after pressing share', async () => {
    ...
    expect(window.open).toHaveBeenCalledWith(
      'https://play.golang.org/p/abcdefg'
    );
  });
  ...
  it('replaces textarea with formated code after pressing format', async () => {
    ...
    expect(el<HTMLTextAreaElement>('.Documentation-exampleCode').value).toBe(
      '// mocked response'
    );
  });
  ...
  it('escapes example output', async () => {
    mocked(window.fetch).mockResolvedValue({
      json: () =>
        Promise.resolve({
          Events: [
            { Message: '\u003cinput required\u003e', Kind: 'stdout', Delay: 0 },
          ],
          Errors: null,
        }),
    } as Response);
    ...
    expect(el('.Documentation-exampleOutput').innerHTML).toBe(
      '&lt;input required&gt;'
    );
  });
